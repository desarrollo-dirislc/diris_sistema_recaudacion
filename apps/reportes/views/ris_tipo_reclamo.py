...
from apps.reclamo.models.entidad_reclamo import MESES, EntidadReclamo, MEDIOS
...
def generar_informacion_reporte(request, anio, mes, ris, ipress):
    ...
    list_final_resultado = ['TIPO'] + [0] * len(MEDIOS)
    list_final_total_reclamo = ['TOTAL'] + [0] * len(MEDIOS)

    if ris == 0 and ipress == 0:
        list_final = [['RIS'] + [a[1] for a in MEDIOS]]
    elif ipress == 0:
        list_final = [['IPRESS'] + [a[1] for a in MEDIOS]]
    else:
        ...
        for r in RIS:
            ...
            for rnr in MEDIOS:
                ...
                reclamos_count_resultado = EntidadReclamo.objects.filter(fecha_reclamo__year=int(anio),
                                                                         fecha_reclamo__month=mes,
                                                                         entidad__ris=r[0],
                                                                         medio_presentacion=rnr[0]).count()
    ...
    return {"list_ipress": list_ipress_return, "list_final": list_final_return,
...
def reporte_tipo_reclamo(request):
    update_menu(request)
    template = loader.get_template('reportes/ris_tipo_reclamo.html')

    context = {
        'title': 'N° DE RECLAMOS SEGÚN SU TIPO',
        'anios': ANIOS,
        'meses': MESES
    }
...
@csrf_exempt
def reporte_tipo_reclamo_excel(request):
    if request.method == 'GET':
        ...
        with BytesIO() as b:
            ...
            writer.save()
            filename = 'Tipo_de_reclamos-' + str(mes) + '_' + str(anio)
